...
void print_menu();
bool perform_operation(char); void search_record();
void add_record();
void delete_record();
void update_record();
...
void menu_print() {
	...
	cout << "E: Exit" << endl;
	cout << endl;
	cout << "Enter a Choice {S,A,U,D,E}: " << endl;
}
...
void search_record(){
	...
	if book.search(name) == 0){
		cout << "Could not find a record matchong your search criteria" << endl;
	}
	getchar();
}
...
void update_record(){
	char name[NAME_LENGTH];
	int choice;
	cout << "Please enter the name of the person whose record you want to upadet (press '*' for full list): " << endl;
	cin.ignore(1000,'\n');
	cin.getline(name,NAME_LENGTH);
	...
	if (personcount == 0){
		...
		cout << "Record succesfully updated" << endl;
	}
	getchar();
}
